...
def make_word_groups(vocab_words):
    ...
    for index,each_word in enumerate(vocab_words):
        if index > 0:
            vocab_words[index] = prefix + each_word
...
def remove_suffix_ness(word):
    ...
    """
    replaced_word = word.rstrip('ness')
    if replaced_word.endswith('i'):
        return replaced_word[:-1] + 'y'
...
def adjective_to_verb(sentence, index):
    ...
    sentence_list = sentence.rstrip('.').split()
